...
def main():
    ...
    path = "/Users/mck/Desktop/palmese_research/corr_scripts/lambda_milliquas_chandark_15.dat"
    data = np.loadtxt(path)
    binmin, binmax = (0, 0.255)
    bins = np.linspace(binmin, binmax, num=24)
    fit_bins = np.linspace(binmin, binmax, num=1000)
    fit_bins = fit_bins[fit_bins <= bins[-2]]
    
    #reflecting the distribution across x-axis  for cleaner probdens fit
    reflect_data = -data #issue here????
    ...
    colors = ["#cc"+f"{(153//len(conf_levels) * n)//16}{(153//len(conf_levels) * n) %16//9}"*2 for n in range(len(conf_levels))]
    
    conf_intervals = get_intervals(fit_logax, fit_bins, conf_levels)
    ...
    fig,ax = plt.subplots(2, figsize=(5, 5), dpi=200)
