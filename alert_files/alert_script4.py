...
ENVIRONMENT DETAILS:
    ...
    - install***: gcn-kafka, slackclient, slack_sdk, astropy, astropy-healpix, healpy,*** ligo.skymap (healpy BEFORE ligo.skymap if on macbook), lxml
...
TOP_DIR = '/Users/mck/Desktop/palmese_research/alert_files/'
...
def messageSlack(ep, cands):
    ...
    if cands==[]:
        ...
        for gw in cands:
            ...
            if gw.prob <= 0.9: # <--actual condition # if True: # <--for troubleshooting #
...
def tests():
    ...
    ep = {#A previously crossmatched object which is temporally coincident, but not spatially.
            ...
            'ICRS':SkyCoord(300.97,-68.777, unit=units.degree),
