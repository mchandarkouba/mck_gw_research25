...
def gw_dataFrame(superevents):
    addresses = set()
    for filename in listdir(SKYMAP_DIR):
        b = any(graceID in filename for graceID in superevents) 
        b &= ('multiorder' in filename)
        b &= ('percent' not in filename)
        if b: addresses |= set((SKYMAP_DIR+filename,))
        
    filename = 'event-versions.csv'
    filepath = TOP_DIR + filename
    event_data = pd.read_csv(filepath)
    masses = event_data['total_mass_source']
    f_cover = pd.Series([0.9]*len(masses))
    gw_arr = pd.DataFrame(f_cover, masses)
...
def main():
    ...
    query = f'FAR < {far}' #add to query?
    conf = 0.90
    
    print('Retrieving GW GraceIDs...')
