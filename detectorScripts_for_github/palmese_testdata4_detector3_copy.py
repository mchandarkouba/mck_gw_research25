...
#includes a searching method for any "detectors of interest", 
#given that the number of detectors used to observe an event equals the "number of interest" parameter
...
############################################################################### Search parameters and color-coding

detectorsInterest = {'H1','L1'}
numberInterest = 1
...
for m in detectorMatch:
    ofInterest = any(n in m.split(',') for n in detectorsInterest) and numberInterest==len(m.split(','))
...
plt.title(f'area (70% CL) vs dL for {numberInterest} of {detectorsInterest} detectors')
