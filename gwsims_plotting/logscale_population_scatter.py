...
#and ssh-retrieved detector data in .npy
...
eventPath = r"/path/to/data.csv"
detectorPath = r"/path/to/other/data.npy"
...
def generateMain(eventPath, detectorPath):
    ...
    param = [ # set(*detectors), condition, set(*number), dotColor, opacity
                [set(['a','b','c','d']), 'any', set([1]), 'gold', 0.5],
                [set(['a','b']), 'uniques', set([2,3]), 'green', 0.5],
                [set(['a','b']), 'all', set([2,3,4]), 'purple', 0.5],
                [set(['c','d']), 'all', set([2]), 'blue', 0.5]
            ]
    # a, b, c, d are placeholders
    ...
    mapping = colorPops(param, detectorMatch, **mapping)

    args = (D, A_70, fig, axs, param, mapping)
    scatterHists(*args)
    
    fig.suptitle(f"""Title""", fontsize=24, fontweight='semibold')
    axs['scatter'].set_xlabel('x axis', fontsize=20)
    axs['scatter'].set_ylabel('y axis', fontsize=20)
